...
<body>
	...
	<section>
		...
		<div class="container">
			<form class="form-horizontal form-width center-auto" method="post"
				...
				<div id="inputId" class="form-group has-feedback">
					...
					<div class="col-sm-10">
						<input type="text" class="form-control"
							...
							placeholder="영문 포함 6-13자리를 입력해주세요" maxlength="13"> <span
							id="inputIdIcon" class="glyphicon form-control-feedback"
							aria-hidden="true"></span> <span id="inputIdStatus"
							class="sr-only"></span>
					</div>
					<span id="spanId"></span>
				</div>
				...
				<div id="inputPw" class="form-group has-feedback">
					...
					<div class="col-sm-10">
						<input type="password" class="form-control"
							aria-describedby="inputPwStatus" id="password" name="password"
							placeholder="영문 숫자포함 6자리이상 입력해주세요" maxlength="13"> <span
							id="inputPwIcon" class="glyphicon form-control-feedback"
							aria-hidden="true"></span> <span id="inputPwStatus"
							class="sr-only"></span>
					</div>
				...
				<div id="inputCheckPw" class="form-group has-feedback">
					...
					<div class="col-sm-10">
						<input type="password" class="form-control" id="checkPassword"
							name="checkPassword" placeholder="비밀번호를 다시 입력해주세요" maxlength="13">
						<span id="inputPwCheckIcon"
					...
					<div>
						...
						<div class="input-group col-sm-6">
							<span class="input-group-addon">@</span> <input type="text"
								...
								placeholder="gmail.com" aria-describedby="sizing-addon2">
						</div>
					</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<input type="text" class="form-control" id="user_birthday"
							name="user_birthday" placeholder="ex) 1995-01-30" maxlength="10">
					</div>
				...
				<div class="form-group">
					...
					<label class="radio-inline padding-left"> <input
						type="radio" name="user_gender" checked="checked" value="M">
						남자
					</label> <label class="radio-inline padding-left"> <input
						type="radio" name="user_gender" value="G"> 여자
					</label>
				</div>
				<hr>
				<!-- 전화번호 -->
				<div class="form-group">
					...
					<div class="col-sm-10">
						<input type="text" class="form-control" id="user_phone"
							name="user_phone" placeholder="' - ' 제외하고 입력해주세요." maxlength="11">
					</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-5">
						<input type="number" class="form-control" id="user_post"
							name="user_post" readonly="readonly">
					</div>
					<div>
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<input type="text" class="form-control" id="user_address"
							name="user_address" readonly="readonly">
					</div>
				</div>
				<!-- 상세주소 -->
				<div class="form-group">
				...
				<div class="form-group" align="center">
					<button type="submit" class="btn btn-primary btn-lg">회 원 가
						입</button>
				</div>
			</form>
		</div>
	...
	<script type="text/javascript">
								...
								if (data == 0) {
									...
									if ($('#user_id').val() != '') {
										...
										if (!userIdCheck.test($('#user_id')
												.val())) {
											$('#spanId').text(
													"5자리 이상인 영문과 숫자만 입력해주세요.")
													.css("color", "red");
											$('#inputId').removeClass(
													'has-success').addClass(
													'has-error');
											$('#inputIdIcon').removeClass(
			...
			$('#password').blur(
					function() {
						...
						if (!passwordCheck.test($('#password').val())) {
							$('#spanPassword').text(
									"8자리 이상인 특수문자, 영문, 숫자만 입력가능합니다.").css(
									"color", "red");
							$('#inputPw').removeClass('has-success').addClass(
									'has-error');
							$('#inputPwIcon').removeClass('glyphicon-ok')
						...
						// 입력한 텍스트의 유효성 체크 (true)
							...
							$('#spanPassword').text("");
							$('#inputPw').removeClass('has-error').addClass(
									'has-success');
							$('#inputPwIcon').removeClass('glyphicon-remove')
									.addClass('glyphicon-ok');
							pwFlag = true;
						}
					})
			...
			$('#checkPassword').blur(
					function() {
						...
						if ($('#password').val() != $('#checkPassword').val()) {
							$('#spanCheckPassword').text("입력한 패스워드가 다릅니다.")
									.css("color", "red");
							$('#inputCheckPw').removeClass('has-success')
									.addClass('has-error');
							$('#inputPwCheckIcon').removeClass('glyphicon-ok')
									.addClass('glyphicon-remove');
							$('#checkPassword').text("");
							pwCheckFlag = false;
						}
						...
						// 입력한 패스워드를 이중체크하여 값이 동일할 시, 확인 디자인으로 변경한다 
							...
							$('#inputCheckPw').removeClass('has-error')
									.addClass('has-success');
							$('#inputPwCheckIcon').removeClass(
									'glyphicon-remove')
									.addClass('glyphicon-ok');
							pwCheckFlag = true;
						}
		...
		function checkInfo() {
			...
			// 비밀번호 동일입력 확인
			if (!pwCheckFlag) {
				$('#spanCheckPassword').text("비밀번호를 다시 입력해주세요.").css("color",
						"red");
				$('#checkPassword').focus();
				return false;
			}
			...
			if (!nameCheck.test($('#user_name').val())) {
				$('#spanUserName').text("한글과 2~10자리를 입력해주세요.").css("color",
						"red");
				$('#user_name').focus();
				return false;
			}
			...
			if (!emailCheck.test($('#user_email').val())) {
				$('#spanUserEmail').text("이메일 아이디를 확인해주세요.")
						.css("color", "red");
				$('#user_email').focus();
				return false;
			}
			...
			if (!birthdayCheck.test($('#user_birthday').val())) {
				$('#spanBirthday').text("생년월일과 형식을 확인해주세요.")
						.css("color", "red");
				$('#user_birthday').focus();
				return false;
			}
